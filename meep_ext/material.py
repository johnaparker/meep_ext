...
def drude_lorentz_material(freq, gamma, sigma, eps_inf=1):
    ...
    for i in range(Npoles):
        ...
        susc.append(func(frequency=freq[i], gamma=gamma[i], sigma=sigma[i]))

    material = meep.Medium(epsilon=eps_inf, E_susceptibilities=susc)
    return material

def lorentz_material(freq, gamma, sig,a, eps_inf=1):
    ...
    susc = [func(frequency=freq[i], gamma=gamma[i], sigma=sigma[i]) for i in range(Npoles)]
...
def Au():
    ...
    return drude_lorentz_material(f, gam, sig, eps_inf)
